...
def rvc_models_root():
    configured = os.environ.get("VOICE_REVOLVER_RVC_MODEL_DIR")
    if configured:
        return configured
    model_root = os.environ.get("VOICE_REVOLVER_MODEL_DIR")
    if model_root:
        return os.path.join(model_root, "rvc")
    return os.path.join(now_dir, "rvc", "models")


def rvc_model_path(*parts):
    configured_path = os.path.join(rvc_models_root(), *parts)
    if os.path.exists(configured_path):
        return configured_path
    return os.path.join(now_dir, "rvc", "models", *parts)


base_path = rvc_model_path("formant", "stftpitchshift")
...
def load_embedding(embedder_model, custom_embedder=None):
    embedder_root = rvc_model_path("embedders")
    embedding_list = {
